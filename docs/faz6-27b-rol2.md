...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Qwen3.8-27B-IQ4_XS × cli × üretim öneği × rol(rol) | 50 | 98% [90%–100%] | 85% | — | 0 | 0 | 0 (4) | 0 | 0 | 15 | 0.20 | 0.39 |
| Qwen3.8-27B-IQ4_XS × cli × üretim öneği × rol(rol-aday) | 50 | 92% [81%–97%] | 88% | — | 0 | 0 | 0 (4) | 0 | 0 | 13 | 0.20 | 0.37 |
| Qwen3.8-27B-IQ4_XS × cli × kontrol × üretim öneği × rol(rol) | 18 | 94% [74%–99%] | — | — | 0 | 0 | 0 (0) | 0 | 0 | 41 | 0.13 | 0.69 |
| Qwen3.8-27B-IQ4_XS × cli × kontrol × üretim öneği × rol(rol-aday) | 18 | 100% [82%–100%] | — | — | 0 | 0 | 0 (0) | 0 | 0 | 39 | 0.13 | 0.68 |
| Qwen3.8-27B-IQ4_XS × cli × halusinasyon × üretim öneği × rol(rol) | 15 | 73% [48%–89%] | 100% | — | 0 | 0 | 0 (0) | 0 | 0 | 15 | 0.59 | 0.76 |
| Qwen3.8-27B-IQ4_XS × cli × halusinasyon × üretim öneği × rol(rol-aday) | 15 | 100% [80%–100%] | 100% | — | 0 | 0 | 0 (0) | 0 | 0 | 9 | 0.65 | 0.76 |
| Qwen3.8-27B-IQ4_XS × cli × bellek × üretim öneği × rol(rol) | 17 | 100% [82%–100%] | 100% | — | 0 | 0 | 0 (7) | 0 | 0 | 11 | 0.81 | 0.95 |
| Qwen3.8-27B-IQ4_XS × cli × bellek × üretim öneği × rol(rol-aday) | 17 | 94% [73%–99%] | 91% | — | 0 | 0 | 0 (7) | 0 | 0 | 10 | 0.88 | 1.02 |
| Qwen3.8-27B-IQ4_XS × json × üretim öneği × rol(rol) | 50 | 96% [87%–99%] | 91% | — | 0 | 0 | 0 (4) | 0 | 0 | 24 | 0.19 | 0.46 |
| Qwen3.8-27B-IQ4_XS × json × üretim öneği × rol(rol-aday) | 50 | 88% [76%–94%] | 88% | — | 0 | 0 | 0 (4) | 0 | 1 | 22 | 0.21 | 0.46 |
| Qwen3.8-27B-IQ4_XS × json × kontrol × üretim öneği × rol(rol) | 18 | 89% [67%–97%] | — | — | 0 | 0 | 0 (0) | 0 | 0 | 40 | 0.13 | 0.67 |
| Qwen3.8-27B-IQ4_XS × json × kontrol × üretim öneği × rol(rol-aday) | 18 | 100% [82%–100%] | — | — | 0 | 0 | 0 (0) | 0 | 0 | 38 | 0.13 | 0.67 |
| Qwen3.8-27B-IQ4_XS × json × halusinasyon × üretim öneği × rol(rol) | 15 | 87% [62%–96%] | 100% | — | 0 | 0 | 0 (0) | 3 | 0 | 20 | 0.58 | 0.81 |
| Qwen3.8-27B-IQ4_XS × json × halusinasyon × üretim öneği × rol(rol-aday) | 15 | 100% [80%–100%] | 100% | — | 0 | 0 | 0 (0) | 0 | 0 | 18 | 0.64 | 0.85 |
| Qwen3.8-27B-IQ4_XS × json × bellek × üretim öneği × rol(rol) | 17 | 100% [82%–100%] | 100% | — | 0 | 0 | 0 (7) | 0 | 0 | 18 | 0.80 | 1.00 |
| Qwen3.8-27B-IQ4_XS × json × bellek × üretim öneği × rol(rol-aday) | 17 | 88% [66%–97%] | 100% | — | 0 | 0 | 0 (7) | 0 | 0 | 19 | 0.87 | 1.09 |
...
| Qwen3.8-27B-IQ4_XS × cli × üretim öneği × rol(rol-aday) | 93% | 100% | 57% | 100% | 100% | 100% |
...
| Qwen3.8-27B-IQ4_XS × cli × halusinasyon × üretim öneği × rol(rol-aday) | — | — | — | 100% | 100% | — |
...
| Qwen3.8-27B-IQ4_XS × json × üretim öneği × rol(rol-aday) | 93% | 100% | 29% | 100% | 100% | 100% |
...
| Qwen3.8-27B-IQ4_XS × json × bellek × üretim öneği × rol(rol-aday) | 100% | — | — | 67% | 100% | 100% |
...
| Qwen3.8-27B-IQ4_XS × cli × üretim öneği × rol(rol-aday) | 92% (50) | — | — |
...
| Qwen3.8-27B-IQ4_XS × cli × halusinasyon × üretim öneği × rol(rol-aday) | — | 100% (15) | — |
...
| Qwen3.8-27B-IQ4_XS × json × üretim öneği × rol(rol-aday) | 88% (50) | — | — |
...
| Qwen3.8-27B-IQ4_XS × json × bellek × üretim öneği × rol(rol-aday) | — | 91% (11) | 83% (6) |
...
**Qwen3.8-27B-IQ4_XS × cli × üretim öneği × rol(rol-aday)** — 13 yanıt:

- `tek-01`: `Şu an saat dokuz, sabah.`
- `eks-03`: `Hangi cihazı uyandırmamı istersin?`
- `eks-04`: `Tabii, ne zaman ve ne için hatırlatmak istersin?`
- `eks-06`: `Hangi kişiyi silmemi istersin?`
- `eks-07`: `Tabii, ekleyelim. Kişinin adını söyleyebilir misin? Telefon numarası da varsa onu da ver, birlikte kaydedeyim.`
- `yok-01`: `Rica ederim, memnun oldum. Başka bir konuda yardımcı olmamı istersen buradayım.`
- `yok-02`: `İyiyim, teşekkürler. Sen nasılsın?`
- `yok-03`: `Neden programcılar karanlıkta kalır? Çünkü siyahla beyaz arasındaki farkı göremezler.`
- `yok-04`: `Paris.`
- `yok-05`: `Tamam, sorun değil. Başka bir şey lazım olursa buradayım.`
- `yok-06`: `Ben Mayen. Bu makinede çalışan sesli asistanım. Notlarını tutarım, rehberini yönetirim, ders programını söylerim, zamanlanmış hatırlatmalar kurarım, hava durumunu sorarım ve sistemin genel durumuna ba`
- `yok-07`: `Dört eder.`
...
- `kon-08`: `Maalesef ses seviyesini değiştiremiyorum. Bu ayarı cihazının kendi ses kontrollerinden yapman gerekiyor.`
- `kon-09`: `Evet, bir şehir adı verirsen güncel hava durumunu kontrol edebilirim. Hangi şehri merak ediyorsun?`
- `kon-10`: `Hatırlatıcı kurduğunda, belirttiğin saatte makine seninle konuşur ve yazdığın mesajı sesli okur. Yani o an orada olmasan bile, sistem seni o konuda uyarmak için hazırda bekler. İstersen mevcut hatırla`
...
- `kon-12`: `Rehber, bu makinede saklanan kişilerin ad ve telefon numaralarını tutan küçük bir listedir. İstersen kişi ekleyebilir, aradığın isme göre listeleyebilir ya da var olan birinin numarasını güncelleyebil`
...
- `kon-15`: `Anladım, yarın hava durumu güzel olursa piknik planınız var. İstersen yarın için hava durumunu kontrol edebilirim ya da bir hatırlatma ayarlayabilirim. Ne yapmamı istersin?`
- `kon-16`: `Anladım, Ali'nin iyi olduğunu not ettim. Başka bir şey lazım mı?`
- `kon-17`: `Evet, zaman gerçekten çok hızlı akıyor. Bugün Pazartesi ve saat dokuz. Günün nasıl geçiyor?`
- `kon-18`: `Anlıyorum, bazen motivasyon gelmiyor. Ama belki küçük bir başlangıç yaparsan işin kolaylaşır. Bugün programında neler var, bakayım mı?`
...
**Qwen3.8-27B-IQ4_XS × cli × halusinasyon × üretim öneği × rol(rol-aday)** — 1 yanıt:
...
- `bel-05`: `Ankara'da oturuyorsun.`
- `bel-10`: `ev-pc demiştin.`
- `bel-12`: `Evet, pazartesi günleri saat 10'da derse girdiğini söylemiştin.`
...
**Qwen3.8-27B-IQ4_XS × json × üretim öneği × rol(rol-aday)** — 11 yanıt:

- `tek-01`: `Şu an saat dokuz.`
- `eks-03`: `Hangi cihazı uyandırmamı istersin? Lütfen cihazın adını söyle.`
- `eks-07`: `Tamam, ekleyeceğim kişinin adını söyle. Telefon numarası da varsa onu da ver.`
- `yok-01`: `Rica ederim, memnun oldum. Başka bir konuda yardımcı olabilir miyim?`
- `yok-02`: `İyiyim, teşekkürler. Sen nasılsın?`
- `yok-03`: `Neden programcılar karanlıkta kalır? Çünkü siyahla beyaz arasındaki farkı göremiyorlar!`
- `yok-04`: `Fransa'nın başkenti Paris'tir.`
- `yok-05`: `Tamam, sorun değil. Başka bir şey lazım olursa buradayım.`
- `yok-06`: `Ben Mayen. Bu makinede çalışan, kendi verisini yöneten sesli asistanım. Sana rehberini düzenlemekte, notlar tutmakta, ders programını ve hava durumunu sorgulamakta, zamanlanmış hatırlatmalar kurmakta `
- `yok-07`: `Dört eder.`
- `yok-08`: `Tamam, bundan sonra biraz daha yavaş ve net konuşacağım. Anlayamadığın bir yer olursa tekrar sormaktan çekinme.`
...
- `kon-01`: `Günaydın, saat dokuzda iyi geceler demek biraz erken. Bugün nasıl geçiyor?`
- `kon-02`: `Buyurun, size nasıl yardımcı olabilirim?`
- `kon-03`: `Tamam, bekliyorum.`
- `kon-04`: `Rica ederim, iyi günler!`
- `kon-05`: `Evet, dün akşam gerçekten çok güzel geçmişti.`
- `kon-06`: `Bilgisayarlar aslında çok basit bir mantıkla çalışır. İçerisindeki işlemci, saniyede milyarlarca küçük evet ya da hayır sorusuna cevap verir. Bu sorular elektrik sinyalleriyle taşınır ve birleşerek ya`
- `kon-07`: `Tamam, buradayım. Derin bir nefes al. Sana nasıl yardımcı olabilirim? Bir şeyi birlikte çözeriz ya da sadece dinlenirsin, sen bilirsin.`
- `kon-08`: `Maalesef ses seviyesini değiştirecek bir aracım yok. Bu ayarı cihazın kendi menüsünden yapman gerekiyor.`
- `kon-09`: `Evet, bir şehir adı verirsen güncel hava durumunu kontrol edebilirim. Hangi şehri merak ediyorsun?`
- `kon-10`: `Hatırlatıcı kurduğumda, belirttiğin saatte makinede bir zamanlanmış görev oluşturuluyor. O an geldiğinde sana hatırlatma mesajını iletiyorum. Görevler listesinde bekleyen tüm hatırlatıcıları görebilir`
- `kon-11`: `Evet, notların bu makinede senin verilerin olarak saklanıyor. Dışarıya gönderilmiyor ya da başkalarıyla paylaşılmıyor; yalnızca senin erişiminle kalıyorlar.`
- `kon-12`: `Rehber, bu makinede saklanan kişilerin ad ve telefon numaralarını tutan bir listedir. İstersen kişi ekleyebilir, aradığın isme göre listeleyebilir ya da var olan bir kaydın numarasını güncelleyebilirs`
- `kon-13`: `Tamam, sorun değil. İstersen başka bir konuda yardımcı olabilirim.`
- `kon-14`: `Tamam, anlaşıldı. Başka bir konuda yardımcı olmamı istersen buradayım.`
- `kon-15`: `Anladım, yarınki havaya göre karar vereceksiniz. İstersen yarın sabah hava durumunu kontrol edip sana haber verebilirim.`
- `kon-16`: `Anladım, Ali'nin iyi olduğunu not ettim. Başka bir şey lazım mı?`
- `kon-17`: `Evet, zaman gerçekten çok hızlı akıyor. Bugün Pazartesi ve saat dokuzda yeni bir haftaya başlıyorsun.`
- `kon-18`: `Anlıyorum, bazen motivasyon gelmiyor. Ama belki küçük bir başlangıç yaparsan işin daha kolay olur. Bugün hangi dersler var?`
...
- `hal-15`: `Rica ederim, memnun oldum!`
...
**Qwen3.8-27B-IQ4_XS × json × bellek × üretim öneği × rol(rol-aday)** — 4 yanıt:
...
### Qwen3.8-27B-IQ4_XS × cli × üretim öneği × rol(rol-aday) — 8/50

- `tek-01` (tek tool): date_time beklenirken çağrı üretilmedi — çıktı: `Şu an saat dokuz, sabah.`
- `tek-07` (tek tool): target: 'masaüstü' beklenirken 'masaüstü bilgisayar' geldi — çıktı: `<tool> wake_on_lan --target masaüstü bilgisayar`
- `tek-12` (tek tool): day: '2' beklenirken '3' geldi — çıktı: `<tool> course_schedule --day 3`
- `eks-01` (eksik argüman): düz metin beklenirken weather çağrıldı — çıktı: `<tool> weather --city İstanbul`
- `eks-02` (eksik argüman): düz metin beklenirken note_search çağrıldı — çıktı: `<tool> note_search`
...
### Qwen3.8-27B-IQ4_XS × cli × halusinasyon × üretim öneği × rol(rol-aday) — 0/15
...
### Qwen3.8-27B-IQ4_XS × json × üretim öneği × rol(rol-aday) — 10/50
...
- `eks-04` (eksik argüman): düz metin beklenirken date_time çağrıldı — çıktı: `<tool> {"name": "date_time", "arguments": {}}`
...
### Qwen3.8-27B-IQ4_XS × json × bellek × üretim öneği × rol(rol-aday) — 2/17
